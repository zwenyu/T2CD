    ...
    for (g in 0:1){
      for (i in 0:1){
        ...
        for (m in 1:M){
          ...
          res_step = t2cd_step(dat_m, tau.range = c(5, 45), use_arf = F, t_resd = T)
          ptime = proc.time() - s
          sink(NULL)
          ...
          res_sigmoid = t2cd_sigmoid(list(res=matrix(dat_m$res,1), tim=matrix(dat_m$tim,1)),
                                     tau.range = c(5, 45), init.tau = c(10, 25, 40), t_resd = T)          
          ptime = proc.time() - s
...
# save.image('./Application/Univariate/bsc.RData')
save.image('./Application/Univariate/bsc_tdist.RData')
stopCluster(cl)
