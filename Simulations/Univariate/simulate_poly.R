...
# regime 1 generated by polynomial
...
# Initiate cluster
outfilename = paste('./Simulations/Univariate/polysim', phip, piq, '.txt', sep = '')
  ...
  for (i in 1:dN){
    for (j in 1:tauN){
      sim_ij = sim.simple(tau.percent=taupercent_list[j], d=d_list[i], phip=phip, piq=piq,
                          hetero1=hetero1, regime1='poly', seed=k)
...
save.image(paste('./Simulations/Univariate/simulate_poly', hetero1, phip, piq,'.RData',
                 sep = ''))
stopCluster(cl)
